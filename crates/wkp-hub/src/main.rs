...
//! `wkp-hub`: HTTPS + mutual TLS front end (git http-backend, per-tenant
//! pods), per-tenant bare repo and SQLite index, control plane (design
//! 8). A separate binary target so the laptop `wkp` binary never links
//! the Postgres client or control-plane code (design 3.3).
...
//! **(2026-09-11, ADR-0011/#125)** The original design paired this with
//! an SSH transport (`sshd` + `wkp-shell`); that transport was dropped
...
/// provision) -- read once, here, so every subcommand that touches a
/// tenant's repo doesn't need its own separate configuration story
/// beyond this one env var. Never argv (CLAUDE.md's secrets rule
/// doesn't strictly apply to a plain filesystem path, but there is no
/// reason for it to be per-invocation configurable either -- one hub
/// process, one repos root).
...
/// whether RFC 8628/the container exist yet), not a stub written only
/// to give this task's control-plane functions a real caller. The RFC
/// 8628 HTTP endpoints (M5-2) and the container entrypoint (M5-5) are
/// later, separate ways into the same control plane, not replacements
/// for this one.
fn main() {
