...
fn main() {
    ...
    match command.as_deref() {
        ...
        Some("promote") => {
            ...
            match parse_promote_args(args) {
                Ok(opts) => match run_promote(&opts) {
                    Ok(summary) => println!("{summary}"),
                    Err(msg) => {
        ...
        Some("resolve-conflicts") => {
            ...
            match parse_resolve_conflicts_args(args) {
                Ok(path) => match resolve_modify_delete_conflicts(&path) {
        ...
        Some("bundle") => {
            ...
            match args.next().as_deref() {
                Some("export") => match parse_bundle_export_args(args) {
                    Ok(opts) => match run_bundle_export(&opts) {
                        Ok(summary) => println!("{summary}"),
                        Err(msg) => {
                ...
                Some("import") => match parse_bundle_import_args(args) {
                    Ok(opts) => match run_bundle_import(&opts) {
                        Ok(summary) => println!("{summary}"),
                        Err(msg) => {
...
fn parse_promote_args(mut args: impl Iterator<Item = String>) -> Result<PromoteOptions, String> {
    let mut path = std::env::current_dir().map_err(|e| e.to_string())?;
...
fn read_promote_auto_list(store_path: &Path) -> Vec<String> {
...
/// unsigned.
fn run_promote(opts: &PromoteOptions) -> Result<PromoteSummary, String> {
...
fn parse_resolve_conflicts_args(args: impl Iterator<Item = String>) -> Result<PathBuf, String> {
    let mut path = std::env::current_dir().map_err(|e| e.to_string())?;
    for arg in args {
        match arg.as_str() {
            other if !other.starts_with('-') => path = PathBuf::from(other),
...
/// [`resolve_modify_delete_conflicts`] handles before
/// [`wkp_git::finish_merge`] completes the commit.
fn merge_refs_into_current_branch(
    path: &Path,
    ...
    for git_ref in refs_to_merge {
        ...
        if !clean {
            resolve_modify_delete_conflicts(path)?;
...
fn run_bundle_export(opts: &BundleExportOptions) -> Result<BundleExportSummary, String> {
    let device_id = wkp_git::sync::device_id(&opts.path)?;
...
/// into this device's own branch via [`merge_refs_into_current_branch`] --
/// the same merge/resolve machinery `wkp sync` (M3-4) uses for a network
/// remote, since "fetch from somewhere, then merge every branch that
/// brought in" doesn't care whether "somewhere" was a remote or a bundle
/// file. There is nothing to push back to in the air-gapped case, so this
/// stops after merging (unlike `wkp sync`). A bundle's own re-export of
/// this device's own branch (e.g. re-importing a bundle this same device
/// produced) is skipped rather than merged into itself.
fn run_bundle_import(opts: &BundleImportOptions) -> Result<BundleImportSummary, String> {
...
fn resolve_modify_delete_conflicts(repo_dir: &Path) -> Result<Vec<String>, String> {
...
mod tests {
    ...
    fn seed_inbox_item(dir: &Path, key: &TestKey, scope: &str, body: &str) -> String {
    ...
    fn run_promote_moves_item_with_a_human_signed_commit_and_it_reaches_tier_1() {
        ...
        // still agent-signed.
        run_index(dir).expect("run_index before promote");
        run_materialize(&MaterializeOptions {
            path: dir.to_path_buf(),
            tier: 1,
        })
        ...
        run_index(dir).expect("run_index after promote");
        run_materialize(&MaterializeOptions {
            path: dir.to_path_buf(),
            tier: 1,
        })
    ...
    fn wkp_bundle_import_does_not_grant_tier_0_or_1_to_the_merged_in_item() {
        ...
        run_remember_with_body(&opts_a, "imported content should stay tier 2")
        ...
        run_remember_with_body(&opts_b, "local content on B").expect("remember on B");
        ...
        run_index(device_b.path()).expect("run_index after import");
        for tier in [0u8, 1] {
            run_materialize(&MaterializeOptions {
                path: device_b.path().to_path_buf(),
                tier,
