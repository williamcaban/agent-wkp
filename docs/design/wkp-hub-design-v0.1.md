...
**Atomic apply on the read path.** Harnesses read `tier0.md` at session start and `index.db` on every search, possibly while a sync or re-index is running. `tier0.md` (and any future flat tier file) is always written to a temporary file in the same directory and swapped in with `rename(2)`, so a reader never sees a torn file — same discipline as Mutagen's stage-then-apply. `index.db` is the one named exception (ADR-0002, accepted): its incremental writes go directly against the file inside a real SQLite transaction instead, since SQLite's own transaction log already gives the same "never torn, rolls back cleanly on failure" guarantee for a file that has an engine underneath it. A full rebuild (`build_index`) still uses temp-file-then-rename, since there's no existing file to write a transaction against yet.
...
- **Known bug, deliberately deprioritized (issue #136, decision 2026-09-14):** on Debian/Ubuntu, `git`'s bundled libcurl is GnuTLS-linked rather than OpenSSL-linked, and GnuTLS's client-certificate loading path fails outright on this Ed25519 key/certificate (`error reading X.509 key or certificate file`). Registration itself still succeeds (a plain HTTPS POST, no client cert), but every subsequent `git push`/`fetch` over mTLS fails on a real Ubuntu or Debian machine today. The likely fix is re-keying the shared signing/mTLS identity to ECDSA P-256 (confirmed working against both OpenSSH signing and GnuTLS-curl) rather than a GnuTLS-specific workaround, but that is its own ADR-worthy decision, not yet made. CI works around it (`deploy/hub/test-mtls-integration.sh` routes the test's git client through the hub image's own OpenSSL-linked git); real Ubuntu/Debian users get no such workaround.
...
**Linux half done, merged 2026-09-14 (M6-1, issue #170, PR #189); macOS half not yet built (M6-2, issue #171).**

- **Linux (done):** `crates/wkp-cli/src/sandbox.rs` -- a [Landlock](https://docs.kernel.org/userspace-api/landlock.html) ruleset restricting *write*-class filesystem access (`AccessFs::from_write`, read/execute untouched) to the caller's resolved store path, applied by `remember`/`promote`/`forget`/`index` once they've resolved it (covers the index and git's object directory for free, since both live under the store path); plus a `seccomp` filter (via `seccompiler`, the same crate AWS Firecracker uses) denying a short, specific list of syscalls no normal `wkp` operation has a legitimate reason to call (`ptrace`, `process_vm_{read,write}v`, `mount`/`umount2`, `pivot_root`, `reboot`, `kexec_load`, module load/unload, `acct`, `swapon`/`swapoff`, `iopl`/`ioperm` on x86_64 only, `personality`, `bpf`) rather than an allow-list of everything the binary needs. Best-effort by design: an unsupported/older kernel logs to stderr and continues rather than refusing to run. The read-only hot path (`search`/`context`/`materialize`) is untouched. No `unsafe` was needed (`wkp-cli`'s `#![forbid(unsafe_code)]` stayed intact); verification is correspondingly split between CI (two hidden subcommands driven as real subprocesses of the compiled binary, proving the write restriction and post-filter ordinary operations both work end-to-end) and hand-verification documented in `sandbox.rs`'s own doc comments (that `ptrace` itself gets blocked, confirmed via a standalone forked-child program kept outside the crate, since observing that directly needs `unsafe` FFI this crate forbids).
- **macOS (not yet built):** `sandbox-exec` profiles are deprecated but still functional for CLI tools and are used where available, with the hardened-runtime entitlements applied to the signed binary **[unverified for the specific macOS version in use; validate at build time]**.
...
**Updated 2026-09-14**: the Process row now describes a real fix (issue #159), not a gap -- replacing the two-transport/shared-UID table 8.1/8.2's own "Superseded" annotations already flagged as out of date. The Observability row below still describes a real gap between this design and what's built, not stale prose.
...
| Tenant | Container-runtime pod per tenant (ADR-0009/0010, `PodOrchestrator`: podman today, Kubernetes reserved, ADR-0012) -- not a Unix-UID switch; one persistent storage directory per tenant (bare repo *and* index, ADR-0014) bind-mounted outside the pod's own ephemeral filesystem, as a whole, into every container in that tenant's pod; `receive.fsckObjects=true`/`transfer.fsckObjects=true` so malformed objects are rejected on push. Per-tenant resource/cgroup quotas are **not yet decided** (ADR-0010's own open item) |
| Process | **Fixed (issue #159).** A tenant's pod runs two containers, not one: the network-facing `wkp-hub serve-tenant` (`git http-backend`), and a separate `wkp-hub index-worker` that polls the same bind-mounted repo for its own indexing, with no way to touch the network at all -- `--network none` (its own network namespace, loopback only; Podman-only, no Kubernetes equivalent) plus a seccomp profile denying every socket-related syscall (`securityContext.seccompProfile`'s per-container equivalent exists in Kubernetes, so this half of the control does carry over to a future `KubernetesOrchestrator`). Corrects ADR-0009's original assumption that a same-pod sibling container would get this "for free" from the pod's own network namespace -- see that ADR's 2026-09-14 addendum. Verified by hand against real podman and by `deploy/hub/test-tenant-indexing-isolation.sh`. **Fixed (issue #165, ADR-0014)**: the indexing container's own `index.db` is now host-persisted and visible to the serving container too -- both mount the tenant's whole storage directory, not the bare repo alone |
...
- [OpenSSF Scorecard](https://github.com/ossf/scorecard) runs on every push and PR -- **built** (`.github/workflows/scorecard.yml`). Publishes to the Security tab (push/schedule only, informational); **also an enforcing gate now (issue #178)**: a separate `score-gate` job fails if the aggregate score drops below 4, on push, PR, and the weekly schedule alike. [Allstar](https://github.com/ossf/allstar) itself remains not implemented -- this is a custom threshold check against Scorecard's own JSON output, not Allstar.
...
**M6 in progress (issues #172/#173/#174/#176, `.github/workflows/release.yml`), verified against a real tag (2026-09-14/15).** A release workflow builds `wkp` for `x86_64-unknown-linux-musl` and `aarch64-apple-darwin` twice each, on two independent runners, with `SOURCE_DATE_EPOCH` and `--locked`, failing if the two attempts' hashes don't match (verified: both reproduce byte-identically); signs every binary keylessly with cosign (Fulcio + Rekor, no private key this repo has to protect -- real public transparency-log entries now exist), self-verified with `cosign verify-blob` in the same job; generates SLSA Build L3 provenance via the official generic generator over the exact binaries shipped; generates an SBOM (CycloneDX and SPDX, via Syft against `Cargo.lock`); builds and pushes a signed `linux/amd64` OCI image to `ghcr.io/<owner>/wkp` (a real, public push -- confirmed on the actual registry, not a local ephemeral one). Publishes all of it as a **draft** GitHub Release -- not published for real yet, since Homebrew tap/self-update verification/sandboxing (issues #175, #177, #170-#171) are still open, and a human decides when a draft here actually becomes real.

`aarch64-unknown-linux-musl` is temporarily dropped from this pipeline (William's decision, 2026-09-14): its two independent builds via the `cross` Docker-based toolchain produce genuinely different SHA256 hashes, a real non-determinism not yet root-caused (`rust-ci.yml`'s own cross-compile CI job still builds/smoke-tests it, unaffected). The OCI image is `linux/amd64`-only until that's fixed and `linux/arm64` is re-added.

Getting from "merged, not yet verified" to a real, complete draft release took ten real tag pushes (`v0.1.0-rc1` through `rc10`, per William's go-ahead each time -- pushing tags remained a visible action to the shared repo throughout) and found eight real bugs a code-review pass never would have, each fixed and merged in its own small PR (#193, #195, #196, #197, #198, #199, #200, #201, #202): GitHub Actions expression syntax parsing hyphenated `needs`/`outputs` identifiers as subtraction; `scorecard.yml`'s push trigger silently failing on every push since M6-9 (`ossf/scorecard-action` only supports `push` events on the repo's actual default branch); the SLSA provenance job needing `contents: write` permission, not `read` (a version-pin to an older generator was tried first and was a misdiagnosis, reverted); macOS runners lacking `sha256sum`; the build matrix's `include`+`attempt` keys not cross-producting the way intended, silently dropping 4 of 6 build jobs; `podman manifest add` resolving a bare `localhost/...` reference as an actual registry pull instead of local image storage; `cosign sign` unable to see `podman login`'s credentials (different default credential-file locations); and `gh release create` needing an explicit `--repo` since that job never checks out the source tree. The release profile and size/startup budget below are the other part of this section already true, since they're part of `Cargo.toml` itself, not the release pipeline.
